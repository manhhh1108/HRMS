    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Lịch nghỉ lễ <span>{{ date('Y') }}</span></h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active">Ngày nghỉ</li>
                    </ul>
                </div>
                <div class="col-auto float-right ml-auto">
                    <a href="#" class="btn add-btn" data-toggle="modal" data-target="#add_holiday"><i class="fa fa-plus"></i> Thêm ngày nghỉ</a>
                </div>
            </div>
        </div>
        <!-- /Page Header -->
                    ...
                    <table class="table table-striped custom-table datatable">
                        ...
                        <tbody>
                            ...
                            <td hidden class="id">{{ $item->id }}</td>
                                <td>{{ ++$key }}</td>
                                <td class="holidayName">{{ $item->name_holiday }}</td>
                                <td hidden class="holidayDate">{{ $item->date_holiday }}</td>
                                <td>{{ date('d F, Y', strtotime($item->date_holiday)) }}</td>
                                <td>{{ date('l', strtotime($item->date_holiday)) }}</td>
                                <td class="text-right">
                                    <div class="dropdown dropdown-action">
                                        <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item userUpdate" data-toggle="modal" data-id="{{ $item->id }}" data-target="#edit_holiday"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                            <a class="dropdown-item deleteRecord" data-toggle="modal" data-target="#deleteRecord"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Thêm ngày </h5>
