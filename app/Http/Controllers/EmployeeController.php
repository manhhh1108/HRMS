...
class EmployeeController extends Controller
{
    ...
    public function saveRecordDesignations(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $designations = Designation::where('name', $request->name)->first();
            $department = Department::where('id', $request->department_id)->first();

            if (!$department) {
                session()->flash('error', 'Department does not exist');
                return redirect()->back();
            }
            ...
            if ($designations === null) {
                ...
                DB::rollback();
                session()->flash('error', 'Designation already exists');
                return redirect()->back();
            }
        } catch (\Exception $e) {
            DB::rollback();
            session()->flash('error', 'Failed to add designation');
            return redirect()->back();
        }
    }
    public function updateRecordDesignations(Request $request)
    {
        ...
        try {
            ...
            Designation::where('id', $request->id)->update($designations);
            DB::commit();
            flash()->success('Updated record successfully :)');
            return redirect()->back();
        } catch (\Exception $e) {
